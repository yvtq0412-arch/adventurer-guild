      ...
      <div className="space-y-8 text-sm leading-relaxed text-gray-600">
        ...
        <Section title="第1条（定義）">
          ...
          <ul>
            ...
            <li><strong>「ワーカー」（受注者）</strong>：本サービスを通じて依頼を受注し、作業を行うユーザー。本規約における「ワーカー」は一般的な「受注者」「ワーカー」「フリーランサー」と同義です。</li>
            <li><strong>「取引」（依頼）</strong>：依頼者が本サービス上に投稿する作業依頼。本規約における「取引」は一般的な「依頼」「案件」「タスク」と同義です。</li>
            <li><strong>「ワーカープロフィール」（受注者プロフィール）</strong>：ワーカーの本人確認・スキル・実績をまとめたプロフィール。サービスの受注に必要です。</li>
            <li><strong>「エスクロー」（仮払い・第三者預託）</strong>：取引の安全を確保するため、当社が指定する決済事業者（Stripe, Inc.）を通じて報酬を一時的に預託する仕組み。当社自身が資金を預かるものではありません。作業完了・依頼者の承認後にワーカーへ送金されます。</li>
            <li><strong>「サービス利用料」</strong>：本サービスの利用対価として取引金額から差し引かれる手数料（基本15%、ランクにより10%〜15%で変動）</li>
            <li><strong>「ランク」（受注者の信頼度）</strong>：ワーカーの実績・評価に基づくF〜Sの7段階の信頼度指標。ランクが高いほどサービス利用料が優遇されます。</li>
          </ul>
        </Section>
        ...
        <Section title="第3条（サービスの内容）">
          <ol>
            <li>本サービスは、リアルな作業・サービス（清掃、草取り、引越し、イベント設営等）の依頼者（発注者）とワーカーをマッチングするプラットフォームです。</li>
            <li>当社は仲介者として取引の場を提供するものであり、依頼者とワーカーの間の契約当事者ではありません。</li>
            <li>取引に関する最終的な責任は、依頼者とワーカーの間に帰属します。</li>
        ...
        <Section title="第4条（エスクロー決済）">
          <ol>
            <li>依頼者はサービス投稿時に報酬全額をエスクロー（Stripeによる仮払い・決済預かり）します。</li>
            <li>エスクロー完了後、サービスは「募集中」状態となりワーカーが応募できます。</li>
            <li>ワーカーが作業完了を報告し、依頼者が承認した時点で報酬がワーカーに分配されます。</li>
            <li>分配額は「取引金額 − サービス利用料（サービス利用料 15%〜10%）」です。端数は切り捨て（ギルド側が負担）とします。</li>
            <li>Stripe Connect（振込）を利用するため、ワーカーは Stripe のオンボーディング（本人確認）が必要です。</li>
          </ol>
        </Section>

        <Section title="第5条（サービス利用料）">
          <ol>
            <li>本サービスの利用対価として、取引金額の15%をサービス利用料として徴収します。ワーカーのランクに応じて10%〜15%で変動します。</li>
            <li>手数料率は事前告知の上、変更する場合があります。</li>
            <li>手数料はワーカーの報酬から差し引かれます（依頼者が支払う金額には含まれません）。</li>
            <li>手数料に関する消費税はサービス利用料内に含まれます。</li>
          </ol>
        </Section>
        ...
        <Section title="第6条（源泉徴収）">
          <ol>
            ...
            <li>将来、デザイン・執筆・翻訳等の源泉徴収対象カテゴリが追加された場合は、以下の税率でワーカーへの報酬から控除されます。<br />
            ...
            <li>ワーカーが得た報酬に関する所得税の申告・納付は、ワーカー本人の責任において行ってください。</li>
          </ol>
        </Section>
        ...
        <Section title="第7条（インボイス制度への対応）">
          <ol>
            ...
            <li>適格請求書発行事業者のワーカーは、登録番号を登録することで適格請求書の発行が可能です。</li>
        ...
        <Section title="第8条（キャンセル・返金）">
          <table>
            ...
            <tbody>
              <tr>
                <td>ワーカー受諾前（ESCROWED）</td>
                <td>全額返金（エスクロー解除）</td>
              </tr>
              <tr>
                <td>作業中（WORK_IN_PROGRESS）</td>
                <td>90%返金、10%はGuildが維持費として保持</td>
              </tr>
              <tr>
        ...
        <Section title="第9条（紛争解決）">
          <ol>
            <li>依頼者・ワーカー間で紛争が生じた場合、まず当事者間での解決を試みてください。</li>
        ...
        <Section title="第10条（禁止事項）">
          ...
          <SubSection title="決済・取引に関する禁止事項">
            <ul>
              ...
              <li>マネーロンダリング（資金洗浄）その他の不正な金銭取引</li>
              <li>架空の依頼を投稿し、共謀者に受注させて資金を移動する行為</li>
              <li>実態のないサービスに対して報酬を支払う行為</li>
              <li>取引制限（金額上限・回数制限）を回避する目的での複数アカウントの使用</li>
            </ul>
          </SubSection>
          <SubSection title="不正利用防止のための取引制限">
            <p>当社は不正利用・マネーロンダリングを防止するため、以下の取引制限を設けています。</p>
            <ul>
              <li>取引実績が3件未満のユーザーは、1回あたりの依頼金額が50,000円以下に制限されます</li>
              <li>取引実績が3件以上のユーザーは、1回あたりの依頼金額が100,000円以下に制限されます</li>
              <li>同一の依頼者と受注者の組み合わせによる取引は、月間3回・月間150,000円を上限とします</li>
              <li>上記の制限は、サービスの健全性を維持するため予告なく変更する場合があります</li>
              <li>不正な取引パターンが検出された場合、当社は取引の停止・アカウント停止・関係当局への通報を行うことがあります</li>
            </ul>
          </SubSection>
        ...
        <Section title="第10条の2（資格・免許が必要な作業について）">
          ...
          <ol>
            <li><strong>依頼者の確認義務：</strong>依頼者は、取引として依頼しようとする作業に資格・免許・許認可が必要かどうかを、事前に自己の責任で確認してください。当社はその判断を保証しません。</li>
            <li><strong>ワーカーの自己申告：</strong>ワーカーは、資格・免許・許認可が必要な作業を受注する場合、当該資格等を現に保有していることを保証するものとします。プロフィールへの資格記載はユーザー自身の自己申告であり、当社はその真正性を審査・保証しません。</li>
            <li><strong>虚偽申告の禁止と責任：</strong>資格・免許を保有していないにもかかわらず、これを保有しているとして受注した場合、依頼者に生じた一切の損害について、当該ワーカーが全責任を負います。</li>
        ...
        <Section title="第11条（知的財産権）">
          <ol>
            ...
            <li>取引でワーカーが制作した成果物の著作権は、別途当事者間での合意に基づきます。</li>
          </ol>
        </Section>
        ...
        <Section title="第12条（免責事項）">
          ...
          <SubSection title="一般的な免責事項">
            <ol>
              ...
              <li>当社の損害賠償責任が認められる場合でも、その上限は当該ユーザーが過去3ヶ月間に支払ったサービス利用料相当額とします。</li>
            </ol>
          </SubSection>
          <SubSection title="依頼内容の法的適合性に関する免責">
            <ol>
              ...
              <li>当社は、禁止サービスガイドラインの策定・公表、利用規約における禁止事項の明示、依頼投稿時および受注時の確認チェックの設置等、法令違反を防止するための合理的な措置を講じていますが、ユーザーがこれらに違反した場合に生じた損害について、当社は一切の責任を負いません。</li>
