...
const SKILL_CATEGORIES: { label: string; icon: string; skills: string[] }[] = [
  ...
  },
  {
    label: '運搬・作業',
    icon: '📦',
    skills: ['引越し補助', '荷物搬入出', '家具組み立て', '家具移動', '倉庫作業', '棚卸し補助', '搬入出'],
  },
  {
    label: '生活・家事',
    icon: '🏠',
  ...
  },
  {
    label: '相談・アドバイス',
    icon: '💬',
    skills: ['キャリア相談', '経営相談', 'IT相談', '生活相談', '農業アドバイス', '起業相談'],
  },
  {
    label: 'その他',
    icon: '🔧',
    skills: ['イベントスタッフ', '看板設置', 'ポスティング', '農業補助', '雪かき', '窓拭き'],
  },
];
  ...
  if (!card) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-gray-600 font-medium mb-2">ワーカープロフィールがありません</p>
        <p className="text-sm text-gray-400 mb-6">
          プロフィール詳細を設定するには、まずワーカープロフィールを申請してください。
        </p>
        <Link
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <nav className="text-sm text-gray-400 mb-6 flex items-center gap-2">
        ...
        <span>/</span>
        <span className="text-gray-600">ワーカープロフィール編集</span>
      </nav>

      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">ワーカープロフィールを編集</h1>
      ...
      {card.status === 'APPROVED' && (
        <div className="bg-green-50 border border-green-100 rounded-xl p-4 mb-6 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-green-600 text-sm font-medium">✅ ワーカープロフィール承認済み</span>
          </div>
