...
const POST_CHECKS: CheckItem[] = [
  ...
  { id: 'fee', label: '取引金額の15%（ランクにより10%〜15%で変動）がサービス利用料として差し引かれること', termsRef: '第5条' },
  { id: 'cancel', label: '作業開始後のキャンセルは10%を差し引いた返金となること', termsRef: '第8条' },
  { id: 'direct', label: 'Guildを介さない直接取引は禁止されていること', termsRef: '第10条' },
  { id: 'prohibited', label: '禁止サービスガイドラインを確認し、白タク行為（人の送迎）・無資格医療行為・無資格電気工事・非弁行為（法律相談）等の法令違反依頼を投稿しないこと', termsRef: '第10条', link: { href: '/prohibited', text: '禁止依頼の詳細を確認' } },
  ...
  { id: 'terms', label: '利用規約・プライバシーポリシー・禁止サービスガイドラインの全文を読み、内容を理解しました' },
];
...
const ACCEPT_CHECKS: CheckItem[] = [
  ...
  { id: 'direct', label: 'Guildを介さない直接取引は禁止されていること', termsRef: '第10条' },
  { id: 'prohibited', label: '禁止サービスガイドラインを確認し、法令違反の依��（白タク・無資���医療・無資格��事・非弁行為等）と知りつつ受注しないこと', termsRef: '第10条', link: { href: '/prohibited', text: '禁止依頼の詳細を確認' } },
...
export function TermsAgreementModal({ mode, onAgreed, onCancel }: Props) {
  ...
  const title = mode === 'post' ? '依頼を作成する前に' : '取引を受注する前に';
  const desc = mode === 'post'
    ? '依頼の作成・エスクロー決済を利用するには、以下の各項目を確認のうえ、すべてにチェックを入れてください。'
    : '取引の受注・報酬の受け取りを行うには、以下の各項目を確認のうえ、すべてにチェックを入れてください。';
